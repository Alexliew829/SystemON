  ...
  for (const comment of post.comments.data) {
    ...
    if (!isFromPage && (message.includes('on') || message.includes('开始'))) {
      if (!systemOnCommented) {
        ...
        systemOnCommented = true
        responseMessages.push(`✅ “on/开始”留言已触发 System On 留言`)
      }
    }

    if (!isFromPage || alreadyProcessed) continue

    // ✅ “zzz”留言，执行 webhook，并回复原留言
    if (message.includes('zzz')) {
      ...
      await fetch(`https://graph.facebook.com/v19.0/${comment.id}/comments`, {
        method: 'POST',
        body: new URLSearchParams({
          message: '🕒 倒数开始，感谢留言 zzz！',
          access_token: process.env.FB_ACCESS_TOKEN,
        }),
      })

      await markAsProcessed(comment.id)
      responseMessages.push(`✅ “zzz”留言已触发 Webhook 并已回复`)
      triggerCount++
    }
  }
